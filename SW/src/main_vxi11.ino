...
class SCPI_handler: public SCPI_handler_interface
{
  public:
    ...
    bool read(char *data, size_t *len, size_t max_len) override
    {
...
void setup() {
  setup_led();
  setup_serial(F("Starting VXI-11.2 socket server GPIB interface..."));
...
void loop() {
  ...
  bool busyflag[NUM_DEVICES];
  busyflag[0] = !vxi_server.available();

  loop_serial(busyflag, NUM_DEVICES);
  // loop_ethernet();
